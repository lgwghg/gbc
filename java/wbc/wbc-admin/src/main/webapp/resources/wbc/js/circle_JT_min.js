...
(function($) {
    ...
    function DrawClock(circle) {
    	$("#Circle_JT_cancvsDom"  + circle.domId).everyTime('1s', circle.domId, function() {
            var X = 0;
            ...
            if (cancvsObj) {
            	...
            	$("#Circle_JT_cancvsDom"  + circle.domId).stopTime(circle.domId);
            	return;
            }
            //console.log(circleObj);
            if (circle.totalValue >= 60) {
                circle.pbColor = "#ff0103";
                circleObj.strokeStyle = "#8c0102";
                X = circle.value * 2 / circle.totalValue;
            ...
            if ((circle.totalValue - circle.value) <= 0) {
            	...
            	$("#Circle_JT_cancvsDom"  + circle.domId).stopTime(circle.domId);
                circleObj.clearRect(0, 0, circle.radius * 3, circle.radius * 3);
                document.getElementById("Circle_JT_innerDom" + circle.domId).innerHTML='';
                if (circle.domId.indexOf('tanjishi_j_') >= 0) { // 加入按钮倒计时处理
                	var roomId = circle.domId.split("_")[2];
                	dealJoinBtn(roomId);
                }
            }
        });
    ...
    function createElementFun(circle) {
        ...
        $("#Circle_JT_innerDom" + circle.domId).everyTime('1s', "innerDom" + circle.domId, function() {
        	circle.value++;
            innerDom.innerHTML = circle.totalValue - circle.value + "s";
            if ((circle.totalValue - circle.value) <= 0) {
                innerDom.innerHTML = "0s";
                //clearInterval(timeOut);
                $("#Circle_JT_innerDom" + circle.domId).stopTime("innerDom" + circle.domId);
            }
        });
    }
