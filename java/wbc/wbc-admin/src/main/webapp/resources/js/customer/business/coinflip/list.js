	...
	}/*,
	{
	    ...
	    resolution : function(value, record, column, grid, dataNo, columnNo) {
	    	...
	    	if(record.status >=0 && record.status<=2){
		    	content+= '<shiro:hasPermission name="coinflipRoom:deleteBatch"><button type="button" onclick="webside.wodota.delModelByOperation(\'/coinflipRoom/deleteBatch.html\',customSearch,\''+record.id+'\')" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>&nbsp;取消游戏</button></shiro:hasPermission>';
	    	}
	    	return content;
	    }
	}*/
...
var dtGridOption = {
    ...
    checkWidth :'37px',
    extraWidth : '37px',
    loadURL : sys.rootPath + '/coinflipRoom/list.html',
