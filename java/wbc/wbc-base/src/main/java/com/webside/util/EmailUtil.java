...
public class EmailUtil {
	...
	public boolean send126Mail(String toMail, String fromName, String subject, String text) throws AuthenticationFailedException, MailException, UnsupportedEncodingException
	{
		...
		Email email = Email.create().from(fromName, USER_126).to(toMail).subject(MimeUtility.encodeText(subject, "UTF-8", "B")).addHtml(text);// .addText(text);
		SmtpServer smtpServer = SmtpServer.create("smtp.exmail.qq.com").authenticateWith(USER_126, PASSWORD_126);
