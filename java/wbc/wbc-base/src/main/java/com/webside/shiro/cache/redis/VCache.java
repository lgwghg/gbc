...
public class VCache {
	...
	final static JedisManager J = (JedisManager) SpringBeanUtil.getBean("jedisManager");
	private VCache() {}
	 ...
	 * @return
	 */
	public static <T> List<T> getVByMap(String mapkey, Class<T> requiredType){
		Jedis jds = null;
		boolean isBroken = false;
		try {
			jds = J.getJedis();
			jds.select(0);
			byte[] mkey = SerializeUtil.serialize(mapkey);
			Map<byte[], byte[]> result = jds.hgetAll(mkey);
			List<T> list = new ArrayList<T>();
			for (Entry<byte[], byte[]> entry : result.entrySet()) {
				T resultObj = SerializeUtil.deserialize(entry.getValue(), requiredType);
				list.add(resultObj);
			}
			return list;
		} catch (Exception e) {
	 ...
	 * @param value   map里的value
	 */
	public static void setVByMap(String mapkey,String key ,Object value, int expireTime){
		Jedis jds = null;
        boolean isBroken = false;
		try {
			jds = J.getJedis();
	 ...
	 * @param value
	 */
	public static void setVBySet(String setKey,String value, int expireTime){
		Jedis jds = null;
        boolean isBroken = false;
		try {
			jds = J.getJedis();
	 ...
	 * @param value
	 */
	public static void setVByList(String listKey,Object value, int expireTime){
		Jedis jds = null;
		boolean isBroken = false;
		try {
			jds = J.getJedis();
