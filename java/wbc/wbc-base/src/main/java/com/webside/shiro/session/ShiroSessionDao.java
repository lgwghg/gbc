 ...
 * 
 */
public class ShiroSessionDao extends CachingSessionDAO implements CacheManagerAware {

	/**
	 * The default active sessions cache name, equal to {@code shiro-activeSessionCache}.
	 */
	public static final String keyPrefix = "shiro:session:cache:";
	...
	/**
	 * Calls {@code super.create(session)}, then caches the session keyed by the returned {@code sessionId}, and then returns this {@code sessionId}.
	 * 
	 * @param session
	...
	public Serializable create(Session session) {
		...
		if (session != null) {
			try {
				cache.updateCached(getKey(sessionId), SerializeUtil.serialize(session), getExpire());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	/**
	 * Returns the cached session with the corresponding {@code sessionId} or {@code null} if there is no session cached under that id (or if there is no Cache).
	 * 
	 * @param sessionId
	...
	/**
	 * Attempts to acquire the Session from the cache first using the session ID as the cache key. If no session is found, {@code super.readSession(sessionId)} is called to perform the actual retrieval.
	 * 
	 * @param sessionId
	...
	public void update(Session session) throws UnknownSessionException {
		...
		if (session instanceof ValidatingSession) {
			if (((ValidatingSession) session).isValid()) {
				...
				try {
					cache.updateCached(getKey(session.getId()), SerializeUtil.serialize(session), getExpire());
				} catch (Exception e) {
					e.printStackTrace();
				}
			} else {
