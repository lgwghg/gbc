/**
 * Created by sunhw on 2016/12/04.
 * Created by sunhw on 2016/12/04.
 */
...
/**
 * 客户端建立连接需要查询历史数据
 * @param  socketId 客户端对象
 * @throws Exception
 */
var smembers = function(socketId)
{
    //查询所有系统通知
    redisClient.smembers("sn:set", function(err, reply)
    {
        //打印并记录日志
        logger.system.debug("client [" + socketId + "] emit all sn:set...");
        //客户端推送通知
        clients[socketId].emit("notice", reply);
    });

    //查询最新盘口获胜动态
    redisClient.smembers("sn:gb:set", function(err, reply)
    {
        //打印并记录日志
        logger.system.debug("client [" + socketId + "] emit all sn:gb:set...");
        //客户端推送通知
        clients[socketId].emit("notice", reply);
    });
    
  	//查询最新翻硬币获胜动态
    redisClient.lrange("sn:coin:list",'0','-1',function(err, reply)
    {
        //打印并记录日志
        logger.system.debug("client [" + socketId + "] emit all sn:coin:list...");
        //客户端推送通知
        clients[socketId].emit("notice", reply);
    });
};
...
io.on('connection', function(socket)
{
    ...
    //清空数据
    clients[socket.id].emit("notice", "-9");
    //此处异步调用 给客户端发送全部数据
    smembers(socket.id);
    //此处采用异步调用
    //clients[socket.id].emit("notice", noticeArray);
